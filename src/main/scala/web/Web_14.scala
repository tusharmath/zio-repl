...
object Web_14 {
  ...
  object HttpMiddleware {
    def of[R](cb: (Http[R], Request[ByteBuf]) => ZIO[R, Unit, Response[ByteBuf]]): HttpMiddleware[R] =
      HttpMiddleware { http =>
        Http(request => cb(http, request))
      }

    def response[R](cb: Response[ByteBuf] => Response[ByteBuf]): HttpMiddleware[R] =
      HttpMiddleware { http =>
        Http(request => http.run(request) map cb)
      }
